...
>[!thm] Nagata-Smirnov度量化定理 (Nagata-Smirnov metrization theorem)
...
>[!lem] 引理1
>设$X$正则且具有可数局部有限基$\mc{B}$, 则$X$正规, 并且$X$的每个闭集都是$G_\delta$集.

>[!pf]
>I.
>设$W$是$X$的一个开集, 我们要证明存在$X$的开集的一个可数族$\set{U_n}$, 使得
>$$W=\bigcup{U_n}=\bigcup{\clos{U_n}}$$
>因为$\mc{B}$是$X$的可数局部有限基, 故可以写作$\mc{B}=\bigcup{\mc{B}_n}$, 其中每个$\mc{B}_n$都是局部有限的. 设$\mc{C}_n$是$\mc{B}$中满足$B\in\mc{B}$且$\clos{B}\subset W$的全体元素$B$组成的族 (对每个$n$), 那么$\mc{C}_n$是局部有限的, 这是因为它是$\mc{B}_n$的子集的一个族, 定义
>$$U_n=\bigcup_{B\in\mc{C}_n}{B}$$
>那么$U_n$是开集, 并且由[[局部有限性|此页面引理1(局部有限性质)]], 
>$$\clos{U_n}=\bigcup_{B\in\mc{C}_n}{\clos{B}}$$
>因此$\clos{U_n}\subset W$, 从而
>$$\bigcup{U_n}\subset\bigcup{\clos{U_n}}\subset W$$
>我们断言等号成立, 给定$x\in W$, 由正则性可知存在$B\in\mc{B}$使得$x\in B$和$\clos{B}\subset W$. 现在, 对某个$n$一定有$B\in\mc{B}_n$, 由定义显然$B\in\mc{C}_n$, 从而$x\in U_n$, 于是$W\subset \bigcup{U_n}$.
>
>---
>II.
...
>[!pf]
>I.
>设$X$正则且有可数局部有限基$\mc{B}$, 则$X$正规, 并且$X$的每个闭子集都是$G_\delta$集. 对于某一个$J$, 我们将把$X$嵌入度量空间$(\R^J,\bar{\rho})$中 (一致度量), 从而说明$X$可度量化.
>
>设$\mc{B}=\bigcup{\mc{B}_n}$, 其中每个$\mc{B}_n$都是局部有限的. 对于每一个整数$n$和$\mc{B}_n$中任一基元素$B$, 选取一个连续函数
>$$f_{n,B}:X\to[0,1 /n]$$
>使得对任意$x\in  B$有$f_{n,B}(x)>0$, 对于$x\notin B$有$f_{n,B}(x_0)=0$. 于是函数族$\set{f_{n,B}}$分离$X$中的点和闭集, 这是因为对于任给的点$x_0$和$x_0$的一个邻域都存在一个基元素$B$使得$x_0\in B\subset U$, 并且对于某一个$n$有$B\in\mc{B}_n$, 从而$f_{n,B}(x_0)>0$, 并且$f_{n,B}(x_0)$在$U$的补集上取零值.
>
>设$J$是$\Z_+\times\mc{B}$的一个子集, 它由全体满足$B\in\mc{B}_n$的偶对$(n,B)$组成, 我们定义
>$$F:X\to[0,1]^J$$ 为
...
>固定$n$, 选取$x_0$的一个邻域$U_n$, 使得其于$\mc{B}_n$中的有限多个成员相交 (局部有限). 也就是说, 当$B$在$\mc{B}_n$上变动时, 函数族除了有限多个以外, 其余的都在$U_n$上恒等于$0$ (定义), 因为每一个$f_{n,B}$连续, 可以选取$x_0$的一个邻域$V_n$包含于$U_n$并且在$V_n$上, 当$B\in\mc{B}_n$时, 每一个保留下的函数$f_{n,B}$最多改变$\epsilon/2$.
>
...
>
>选取$X$的一个度量, 给定$m$, 用$\mc{A}_m$表示全体半径为$1/m$的开球组成的$X$的开覆盖. 由[[加细|该页面引理1(加细)]], 存在可数局部有限的开覆盖$\mc{B}_m$加细$\mc{A}_m$, 显然$\mc{B}_m$中每个元素的直径最多为$2 /m$ (半径最大$1 /m$), 令$\mc{B}=\bigcup_{m\in\Z_+}{\mc{B}_m}$. 因为每一个$\mc{B}_m$都是可数局部有限的, 所以$\mc{B}$也是可数局部有限的, 下面证明$\mc{B}$是$X$的一个基.
>给定$x\in X$和$\epsilon>0$, 我们的目标是证明存在$B\in\mc{B}$并且$B\subset{\mc{B}(x,\epsilon)}$. 先选取$m$, 使得满足$1/m<\epsilon /2$. 由于$\mc{B}_m$覆盖$X$, 可以选取$\mc{B}_m$中的一个元素$B$, 使得$x\in B$. 从而由$x\in B$和$B$的直径最多为$2 /m<\epsilon$可见$B$包含于$\mc{B}(x,\epsilon)$.
