...
>[!pf]
>设Lindelöf空间$X$, $f$是连续映射. 任取$Y\subset f(X)$, $\mc{A}$是$Y$的开覆盖, 那么$\mc{A}$中全体成员在$f$下的原像组成的族$\mc{A}'$是$f^{-1}(Y)$的开覆盖, 那么它有$\mc{S}'$这一可数子覆盖, 那么$\mc{S}'$中全体成员在$f$下的像组成的族$\mc{S}$对应了$\mc{A}$中的可数子覆盖.
