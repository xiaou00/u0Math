...
>[!thm] Tietze扩张定理 (Tietze extension theorem)
...
>进行比较, 由比较判别法可知$g(x)$收敛. 令$s_n=\sum_{i=1}^n{g_i(x)}$<span style="color:rgb(128,128,128)">(前n项和)</span>. 由[[Weierstrass M-判别法]]易知函数列$s_n$是一致收敛的, 从而$g$连续([[一致连续|一致连续定理]]).
>
