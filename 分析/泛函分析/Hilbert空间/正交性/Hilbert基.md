...
>[!pf]
>设$\mc F$是$\mc H$中全体[[规范正交]]集构成的族, 在$\mc F$上按照集合的包含定义偏序, 给定$\mc F$的一个全序子族, 此子集族中所有集合的并包含子集族中的每个子集, 并且仍在$\mc F$中. 由[[Zorn引理]], 存在一个极大的标准正交集, 我们断言每个极大的标准正交基$\set{x_j}$的[[线性张&闭线性张|闭线性张]]是整个空间$\mc H$. 由反证法, 假设存在$y$不属于$\set{x_j}$的闭线性张$X$, 定义
