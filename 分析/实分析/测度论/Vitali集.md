...
$$\mc V=\set{\,\mc p[x]\mid [x]\in[0,1] /\sim\,}$$
其中$\mc p$是选择函数, 由AC能确保其存在性. 我们定义的集合$\mc V$就称为**Vitali集**(Vitali set). 显然Vitali集是**不可数**的: 想象每次从$[0,1]$里选择等价类$[x]$, 这个等价类是可数的.  然而可数的可数并仍然是可数的, 因此一定是不可数并才能使得其构成$[0,1]$的划分. 从而等价类数量不可数, 与之等势的Vitali集也不可数.
