...
>$$\forall A\subset X,\mu^*(A)=\mu^*(A\cap E)+\mu^*(A\setminus E)$$
...
>对于$E\in\Sigma$, 那么$E$满足C-条件, 令$F=\complement_X E$, 对任意$A\subset X$, $A\cap F=A\setminus E$, $A\setminus F=A\cap E$, 从而
>$$\mu^*(A)=\mu^*(A\cap E)+\mu^*(A\setminus E)=\mu^*(A\setminus F)+\mu^*(A\cap F)$$
>再证明可数可加性, 先证明有限情况, 设$A,B\in\Sigma$无交, 我们要证明$A\cup B\in\Sigma$, 由于$A\in\Sigma$, 对任意$S\subset X$,
...
>我们已经证明了$H_n\in\Sigma$. 因此可以应用Carathéodory条件, 对任意集合$S\subset X$,
