...
>[!thm] Riesz-Markov-Kakutani定理 (带号形式)
>设$X$是[[局部紧致]]的[[Hausdorff空间]], $\Lambda$是[[紧支函数]]空间$\csp_c(X,\R)$上的[[有界线性算子|有界线性泛函]], 则在$X$内存在一个包含$X$的全体[[Borel代数|Borel集]]的[[σ-代数]]$\Sigma$, 并且存在$\Sigma$上[[完备测度空间|完备]]的带号[[Radon测度]]$\mu$, 使得对任意$f\in\csp_c(X,\R)$,
>$$\Lambda f=\int_X{f\,\d\mu}$$
