...
>[!def] 自然变换
>设 $\mc C$ 和 $\mc D$ 是[[范畴]], $F:\mc C\to\mc D$ 和 $G:\mc C\to \mc D$ 是两个[[函子]], 一个**自然变换**(natural transformation) $\eta:F\Rightarrow G$ 是一个映射 $\ob{\mc C}\to\mor{\mc D}$:
>$$A\mapsto(\eta_A:F(A)\to G(A)),\quad A\in\ob{\mc C}$$
>使得对 $\mc C$ 的任意态射 $f:A\to B$, $G(f)\eta_A=\eta_BF(f)$ 成立, 即下图表交换:
>```tikz
\usepackage{tikz-cd}
\begin{document}
\large
...
>```
>
>其中 $\eta$ 是一族态射, 以 $\mc C$ 为指标, 元素为连接其在 $\frak D$ 中的两个副本 $F(A)$ 和 $G(A)$.
>若自然变换 $\eta:F\Rightarrow G$ 满足对任意的 $A\in\ob{\mc C}$, $\eta_A:F(A)\to G(A)$ 是一个[[同构]], 则称 $\eta$ 是一个**自然同构**(natural isomorphism), 记作 $F\cong G$.
>
>类似函子范畴的记法, $F$ 到 $G$ 的全体自然变换可记作$\mathsf{Fct}(F,G)$.
